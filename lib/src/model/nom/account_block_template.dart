...
enum BlockTypeEnum { unknown, genesisReceive, userSend, userReceive, contractSend, contractReceive }

extension BlockTypeEnumExtension on BlockTypeEnum {
  int compareTo(BlockTypeEnum blockTypeEnum) => toString().compareTo(blockTypeEnum.toString());
}
...
class AccountBlockTemplate {
  ...
  AccountBlockTemplate({required this.blockType, toAddress, amount, tokenStandard, fromBlockHash, data})
      : version = 1,
  ...
  factory AccountBlockTemplate.receive(Hash fromBlockHash) =>
      AccountBlockTemplate(blockType: BlockTypeEnum.userReceive.index, fromBlockHash: fromBlockHash);

  factory AccountBlockTemplate.send(Address toAddress, TokenStandard tokenStandard, int amount) => AccountBlockTemplate(
      blockType: BlockTypeEnum.userSend.index, toAddress: toAddress, tokenStandard: tokenStandard, amount: amount);
  ...
  AccountBlockTemplate.fromJson(Map<String, dynamic> json)
      : version = json['version'],
        ...
        nonce = json['nonce'],
        publicKey = (json['publicKey'] != null ? BytesUtils.base64ToBytes(json['publicKey']) : [])!,
        signature = (json['signature'] != null ? BytesUtils.base64ToBytes(json['signature']) : [])!;
