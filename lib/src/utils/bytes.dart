...
class BytesUtils {
  static dynamic arraycopy(
      var src, var startPos, var dest, var destPos, var len) {
    for (var i = 0; i < len; i++) {
  ...
  static List<int> bigIntToBytesSigned(BigInt b, int numBytes) {
    var bytes =
        List<int>.filled(numBytes, b.sign < 0 ? 0xFF : 0x00, growable: true);
    var biBytes = encodeBigInt(b);
    var start = (biBytes.length == numBytes + 1) ? 1 : 0;
    var length = min(biBytes.length, numBytes);
    BytesUtils.arraycopy(biBytes, start, bytes, numBytes - length, length);
  ...
  static String bytesToBase64(List<int> bytes) {
    return base64Encode(bytes.where((x) => x != null).cast<int>().toList());
  }
